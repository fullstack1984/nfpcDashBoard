...
class ModelVmp(models.Model):
    ...
    @staticmethod
    def get_values_to_db_data(name_dep, name_dat='dataVmp'):
        my_data = ModelVmp.objects.filter(name_department=name_dep)
        my_list = []
        for i in my_data:
            if i.date_create.strftime('%m/%d/%Y') == date.today().strftime('%m/%d/%Y') and i.name_data == name_dat:                
                my_list.append(i.values_data)
...
class ModelVipis(models.Model):
    ...
    @staticmethod
    def get_values_to_db_data(name_dep, name_dat='dataVipis'):
        my_data = ModelVipis.objects.filter(name_department=name_dep)
        my_list = []
        for i in my_data:
            if i.date_create.strftime('%m/%d/%Y') == date.today().strftime('%m/%d/%Y') and i.name_data == name_dat:                
                my_list.append(i.values_data)
